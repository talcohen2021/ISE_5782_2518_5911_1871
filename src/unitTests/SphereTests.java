...
public class SphereTests {
    ...
    public void testFindIntersections() throws Exception {
        ...
        result = sphere.findIntersections(new Ray(new Point(1, 0, .5), new Vector(0, 0, 2)));
        assertEquals("wrong number of intersection points. Ray starts inside the sphere", result.size(),1);

        
        // TC04: Ray starts after the sphere (0 points)
        result = sphere.findIntersections(new Ray(new Point(1, 3, 4), new Vector(0, 0, 2)));
        ...
        result = sphere.findIntersections(new Ray(new Point(1, 0 ,1), new Vector(1, 0, -2 )));
        ...
        result = sphere.findIntersections(new Ray(new Point(1, 0, 3), new Vector(0, 0, -6)));
        ...
        result = sphere.findIntersections(new Ray(new Point(1, 0, .5), new Vector(0, 0, -2.5)));
        assertEquals("There should be one intersection. Ray starts inside", result.size(), 1);

        // TC16: Ray starts at the center (1 points)
        result = sphere.findIntersections(new Ray(new Point(1, 0, 0), new Vector(5, 5, 5)));
        assertEquals("There should be one intersection.Ray starts at the center", result.size(), 1);
        
        // TC17: Ray starts at sphere and goes outside (0 points)
        result = sphere.findIntersections(new Ray(new Point(1, 0 ,1), new Vector(0, 0, 10 )));
        assertNull("There should be no intersections. Ray starts at sphere and goes outside", result);    
        
        // TC18: Ray starts after sphere (0 points)
        result = sphere.findIntersections(new Ray(new Point(1, 0, 10), new Vector(0, 0, 10)));
        ...
        result = sphere.findIntersections(new Ray(new Point(2, 0, 0), new Vector(0, 0, 1)));
        assertNull("There should be no intersections. Ray starts before the tangent point", result);

        // TC21: Ray starts after the tangent point
        result = sphere.findIntersections(new Ray(new Point(2, 0, 3), new Vector(0, 0, 1)));
        assertNull("There should be no intersections. Ray starts after the tangent point", result);


        // **** Group: Special cases
        // TC19: Ray's line is outside, ray is orthogonal to ray start to sphere's center line
        result = sphere.findIntersections(new Ray(new Point(5, 0, 0), new Vector(0, 0, 1)));
